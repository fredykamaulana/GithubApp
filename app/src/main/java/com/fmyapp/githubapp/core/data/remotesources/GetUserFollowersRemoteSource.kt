...
class GetUserFollowersRemoteSource @Inject constructor(
    private val githubUserService: GithubUserService
) : SafeApiCall() {
    fun getUserFollowers(username: String): Flow<Result<List<UserListItemDto>>> {
        return flow {
            emit(Result.Loading)
            emitAll(safeApiCall { githubUserService.getUserFollowers(username) })
        }
    }
}